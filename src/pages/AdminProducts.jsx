  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl text-white mb-1">All Products</h1>
          <p className="text-steel text-sm">{products.length} total products</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={exportProductsCSV}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-steel text-sm border border-accent-blue/10 hover:border-accent-blue/20 hover:text-white transition-all duration-200"
          >
      ...
      {/* Bulk Actions */}
      {selected.length > 0 && (
        <div className="flex items-center gap-4 px-4 py-3 bg-accent-blue/5 border border-accent-blue/15 rounded-xl">
          <span className="text-accent-electric text-sm font-medium">{selected.length} selected</span>
          <button
            onClick={() => setDeleteModal('bulk')}
            className="flex items-center gap-1.5 px-3 py-1.5 text-red-400 text-sm hover:bg-red-400/10 rounded-lg transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5">
      ...
      <div className="admin-card overflow-hidden">
        {filtered.length > 0 ? (
                ...
                <tr>
                  <th className="w-12">
                    <input
                      ...
                      className="w-4 h-4 rounded border-accent-blue/30 bg-navy-dark text-accent-blue focus:ring-accent-blue/30"
                    />
                  </th>
                   <th>Product</th>
                  <th>Category</th>
                    ...
                    <tr key={product.id}>
                      ...
                      <td>
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-lg bg-navy-dark overflow-hidden flex-shrink-0">
                            {product.images?.[0] ? (
                          ...
                          <div>
                            ...
                            {product.description && (
                              <div className="text-steel/60 text-xs line-clamp-1 max-w-[200px]">{product.description}</div>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="text-steel text-sm font-mono">
                        {product.costPrice ? `₹${product.costPrice}` : '—'}
                      </td>
                      <td className="font-semibold text-accent-electric text-sm font-mono">
                      ...
                      <td>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="w-8 h-8 rounded-lg flex items-center justify-center text-steel hover:text-accent-electric hover:bg-accent-blue/10 transition-all"
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-lg flex items-center justify-center text-steel hover:text-red-400 hover:bg-red-400/10 transition-all"
          ...
          <div className="text-center py-16">
            <svg className="w-14 h-14 text-navy-mid mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1">
              <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/>
            </svg>
            <p className="text-steel text-sm mb-1">No products found</p>
            <p className="text-steel/50 text-xs">Try adjusting your search or filters.</p>
          </div>
        )}
      </div>
        ...
        <div className="modal-overlay" onClick={() => setDeleteModal(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-display text-lg">Confirm Delete</h3>
            ...
            <p className="text-steel/60 text-sm mb-6">This action cannot be undone.</p>
            <div className="flex gap-3">
              <button
