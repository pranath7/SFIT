  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-24 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div ref={textRef} className="space-y-8">
            {/* Eyebrow */}
            <div
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-blue-600/10 bg-blue-50/50"
              style={{ animation: 'fadeUp 0.6s ease-out 0.3s both' }}
            >
              <svg className="w-4 h-4 text-blue-600" viewBox="0 0 24 24" fill="currentColor">
                <polygon points="12,2 15,9 22,9 16,14 18,21 12,17 6,21 8,14 2,9 9,9" />
              </svg>
              <span className="text-blue-700 text-xs font-mono tracking-widest uppercase">Precision Hardware Solutions</span>
            </div>

            {/* Headline */}
            <h1
              className="font-display text-4xl sm:text-5xl lg:text-6xl xl:text-7xl text-slate-900 leading-[1.1]"
              style={{ animation: 'fadeUp 0.8s ease-out 0.5s both' }}
            >
              Where Every Detail{' '}
              <span className="relative">
                <span className="text-blue-600 font-bold">Fits Perfectly</span>
                <svg className="absolute -bottom-2 left-0 w-full h-3" viewBox="0 0 300 12" fill="none" preserveAspectRatio="none">
                  <path d="M0 8 Q75 0, 150 8 Q225 16, 300 8" stroke="#2563eb" strokeWidth="2" fill="none" opacity="0.6"/>
                </svg>
              </span>
              .
            </h1>
            ...
            {/* Subtext */}
            <p
              className="text-slate-600 text-lg sm:text-xl max-w-xl leading-relaxed font-body"
            ...
            >
              <a
                ...
                className="px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-all duration-300 shadow-md hover:shadow-lg text-sm"
                id="hero-explore-cta"
              >
                Explore Products
              </a>
              <a
                ...
                className="group px-8 py-3.5 border border-whatsapp/50 text-whatsapp font-semibold rounded-lg hover:bg-whatsapp/5 transition-all duration-300 flex items-center gap-2 text-sm"
            ...
            >
              {[
                ...
                <div key={badge.label} className="flex items-center gap-2">
                  <span className="text-blue-600 text-xs">{badge.icon}</span>
                  <span className="text-slate-600 text-xs font-mono tracking-wide">{badge.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-float">
        ...
        <div className="w-[1px] h-8 bg-gradient-to-b from-blue-600/40 to-transparent" />
      </div>
    </section>
  );
};
